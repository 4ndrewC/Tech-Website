<?php 
   ...
   if(isset($_POST['g-recaptcha-response'])){
        ...
        //$response=file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=".$google_secret."&response=".$captcha."&remoteip=".$ip);
...
$tarih= date("Y-m-d",strtotime('+7 day'));
					$insertUser = $odb -> prepare("INSERT INTO `users` VALUES(NULL, :username, :password, 0, 0, 0, 0, :referral, 0, 0, 0, 0,  :refered,'','', :tarih,'','')");
					$resultInsert = $insertUser -> execute(array(':username' => $username, ':password' => SHA1(md5($password)), ':referral' => $referral, ':refered' => $referrer, ':tarih' => $tarih));
					if (!$resultInsert) {
	...
	$SQLUpdate = $odb -> prepare("UPDATE `users` SET `login_ip` = :login_ip, `login_useragent` = :login_useragent WHERE `id` = :id");
	$SQLUpdate -> execute(array(':login_ip' => $ip, ':login_useragent' => $_SERVER["HTTP_USER_AGENT"], ':id' => $userInfo['ID']));
	$_SESSION["hash"]=md5(md5($ip.$userInfo['ID'].$_SERVER["HTTP_USER_AGENT"].$userInfo['username']));
