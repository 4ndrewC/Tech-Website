...
router.get("/", async (req, res) => {
    ...
    for(let i = 0; i<tools.length; i++){
        ...
        for(let j = 0; j<list.length; j++){
            if(list[j]["id"]==id){
                subdict["U"] = list[j]["U"];
                subdict["MDL"] = list[j]["MDL"];
                subdict["RA"] = list[j]["RA"];
                subdict["RoTech"] = list[j]["RoTech"];
                subdict["LS"] = list[j]["LS"]
            }
        }
        tags.push(subdict);
        //console.log(subdict);
...
router.post('/', async (req,res) =>{
    var queryResult = "";
    var input = "";
    var checked = new Array(req.body.s1, req.body.s2, req.body.s3, req.body.s4, req.body.s5); 
...
router.get("/:query", async function (req, res) {
    ...
    var checked = [true, true, true, true, true]
    ...
    content.push(tags);
    console.log(content)
    res.render('tl2', {content: content, checked, queryResult})
})
